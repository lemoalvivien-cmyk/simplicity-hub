    ...
    <Body style={main}>
      <Container style={container}>
        ...
        <Section style={bodyPad}>
          ...
          <Text style={text}>Vous avez demandé à modifier votre email sur <strong style={{ color: '#0d1829' }}>{siteName}</strong> de <Link href={`mailto:${email}`} style={link}>{email}</Link> vers <Link href={`mailto:${newEmail}`} style={link}>{newEmail}</Link>.</Text>
          <Text style={text}>Cliquez ci-dessous pour confirmer ce changement :</Text>
          <Button style={button} href={confirmationUrl}>Confirmer le nouvel email →</Button>
          <Text style={hint}>Si vous n'avez pas demandé ce changement, sécurisez votre compte immédiatement.</Text>
        </Section>
        <Hr style={divider} />
