...
echo ""
echo "─── POST-TEST : OÙ OBSERVER LA DÉCISION DE LANCEMENT ────────────────────────"
echo ""
echo "  ÉTAPE A — /admin/payments (vérité billing) :"
echo "    → Onglet 'Billing Proof Chain' (onglet par défaut)"
echo "    → Bloc 'Premier paiement prouvé' : doit passer à OUI ✓ + badge E2E_PROVEN"
echo "    → Bloc 'Décision release suggérée' : doit afficher BILLING_GATE_PASSED"
echo "    → Pipeline billing : tous les stages verts"
echo "    Signal exact qui débloque : full_proof_events >= 1 dans get_billing_proof_summary"
echo ""
echo "  ÉTAPE B — /admin/overview (vérité release gate) :"
echo "    → Bannière Release Gate en haut de page doit passer de :"
echo "        PUBLIC BETA BLOCKED  →  PRIVATE BETA READY  (si 0 capability critique)"
echo "        PUBLIC BETA BLOCKED  →  PRIVATE BETA POSSIBLE  (si capabilities en attente)"
echo "    → Mini-bloc billing 'Premier paiement prouvé : OUI ✓' doit apparaître"
echo "    → Les deux blocs lisent la même source : get_billing_proof_summary RPC"
echo ""
echo "  SIGNAL EXACT DE PROMOTION :"
echo "    - get_billing_proof_summary → full_proof_events >= 1"
echo "    - computeReleaseGate() reçoit billingProof.fullProofEvents >= 1"
echo "    - verdict lève vers PRIVATE_BETA_READY (si 0 capability critique bloquante)"
echo "    - ou PRIVATE_BETA_POSSIBLE (si capabilities encore en attente)"
echo ""
echo "  INTERDICTION ABSOLUE :"
echo "    - full_proof_events = 0 → jamais PRIVATE_BETA_READY, même si code parfait"
echo "    - Le gate recalcule automatiquement à chaque refresh (TanStack Query 90s stale)"
echo ""
echo "═══════════════════════════════════════════════════════════════════"
echo ""
